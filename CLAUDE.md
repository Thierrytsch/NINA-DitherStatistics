...
- `*_top.png`: the Pixel Shift chart renders a sensible cumulative random-walk with the expected point count, centered near the origin, not empty/garbled/clipped/wrong colors; the Settle Time History values and average band look reasonable against the run's `-SettleTime`/`-SettleTimeout` parameters.
...
- Scenario-specific expectations: Profiles' `30_profiles_b.png` shows exactly `-ProfileDitherCount` points and `31_profiles_default_again.png` shows the base profile's full point count again; Clear's screenshots show empty charts and zeroed stats; Restart's screenshots match the pre-restart Baseline finals.
