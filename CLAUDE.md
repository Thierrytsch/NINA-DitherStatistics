...
`PHD2Client` is also where the Dither Settings Optimizer math lives: it collects post-dither `GuideStep` data (`allDitherData`, guarded by `ditherDataLock`), detects "positive periods" of stable guiding, and emits `DitherSettingsRecommendation` via `DitherRecommendationUpdated`. The three profile multipliers (1.5σ / 2.0σ / 3.0σ) are in `RMS_MULTIPLIERS`.
...
The optimizer also writes per-session diagnostic files to `%LocalAppData%\NINA\DitherStatistics\` (`<timestamp>_dither_analysis.txt`, `<timestamp>_positive_periods.txt`) — useful when debugging the recommendation math in `PHD2Client`.
